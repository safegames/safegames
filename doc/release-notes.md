SAFEGAMES Core version *3.1.1* is now available from:  <https://github.com/safegames-project/safegames/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/safegames-project/safegames/issues>

Non-Mandatory Update
==============

SAFEGAMES Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zSGS created before block 1050020

The transition to v2 zSGS and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zSGS to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zSGS v1 will now be able to convert that into SGS and then zSGS v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zSGS stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by SAFEGAMES stakers.
...
### Show progress percent for zsgs reindex operations
...
SAFEGAMES Daemon & Client (RPC Changes)
--------------
...
Here is the long overdue update for SAFEGAMES to let go of OpenSSL in its consensus code. The rationale behind it is to avoid depending on an external and changing library where our consensus code is affected. This is security and consensus critical. SAFEGAMES users will experience quicker block validations and sync times as block transactions are verified under libsecp256k1.

The recent [CVE-2019-0495](https://www.nccgroup.trust/us/our-research/technical-advisory-return-of-the-hidden-number-problem/) brings into question a potential vulnerability with OpenSSL (and other crypto libraries) that libsecp256k1 is not susceptible to.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zSGS code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zSGS elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zsgs reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zSGS created before block 1050020. (presstab)
