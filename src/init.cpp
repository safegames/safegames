...
#include "config/safegames-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("safegames-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "safegames.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "safegamesd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SGS and zSGS money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 61555, 51001));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SGS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SGS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, safegames, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SGS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SAFEGAMES specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-sgsstake=<n>", strprintf(_("Enable or disable staking functionality for SGS inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsgsstake=<n>", strprintf(_("Enable or disable staking functionality for zSGS inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:61555"));
    ...
    strUsage += HelpMessageOpt("-backupzsgs=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSGS minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsgsbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSGS backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizesafegamesamount=<n>     " + strprintf(_("Keep N SGS anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SAFEGAMES Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("safegames-loadblk");
...
/** Sanity checks
 *  Ensure that SAFEGAMES is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SAFEGAMES Core is shutting down."));
...
#endif
    // Make sure only a single SAFEGAMES process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SAFEGAMES Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SAFEGAMES version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService proxyAddr;
        if (!Lookup(proxyArg.c_str(), proxyAddr, 61555, fNameLookup)) {
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService onionProxy;
            if (!Lookup(onionArg.c_str(), onionProxy, 61555, fNameLookup)) {
    ...
    //SAFEGAMES: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SAFEGAMES specific: zerocoin and spork DB's
                ...
                // SAFEGAMES: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SAFEGAMES Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SAFEGAMES Core to complete") << "\n";
        ...
        zwalletMain = new CzSGSWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSGSWallet
        uiInterface.InitMessage(_("Syncing zSGS wallet..."));

        bool fEnableZSgsBackups = GetBoolArg("-backupzsgs", true);
        pwalletMain->setZSgsAutoBackups(fEnableZSgsBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zsgsTracker->Init();
...
//    }
//
//    nAnonymizeSafegamesAmount = GetArg("-anonymizesafegamesamount", 0);
//    if (nAnonymizeSafegamesAmount > 999999) nAnonymizeSafegamesAmount = 999999;
//    if (nAnonymizeSafegamesAmount < 2) nAnonymizeSafegamesAmount = 2;
    ...
    LogPrintf("Anonymize SAFEGAMES Amount %d\n", nAnonymizeSafegamesAmount);
    ...
    /* Denominations
       ...
       For example:
       1SGS+1000 == (.1SGS+100)*10
       10SGS+10000 == (1SGS+1000)*10
    */
