...
CzSGSWallet::CzSGSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsgs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSGSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSGSSeed_deprecated()) {
                    LogPrintf("%s: Updated zSGS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsgs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsgs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSGSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSGSCount(nCountLastUsed);
    else if (!walletdb.ReadZSGSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSGSWallet::Lock()
{
    seedMaster = 0;
}

void CzSGSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSGSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSGS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSGSWallet::LoadMintPoolFromDB()
{
...
void CzSGSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSGSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSGSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsgsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSGSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSGS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsgsTracker which also adds to database
    pwalletMain->zsgsTracker->Add(dMint, true);
...
void CzSGSWallet::SeedToZSGS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSGSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSGSWallet::GenerateDeterministicZSGS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSGSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSGSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
