...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SGS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSGS send
enum ZerocoinSpendStatus {
    ZSGS_SPEND_OKAY = 0,                            // No error
    ZSGS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSGS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSGS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSGS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSGS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSGS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSGS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSGS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSGS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSGS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSGS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSGS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSGS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSGS_SPENT_USED_ZSGS = 14,                      // Coin has already been spend
    ZSGS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSGS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSgsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSGSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsgsAuto) const;
    ...
    */
    ...
    CzSGSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSGSTracker> zsgsTracker;
    ...
    void setZWallet(CzSGSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsgsTracker = std::unique_ptr<CzSGSTracker>(new CzSGSTracker(strWalletFile));
    }

    CzSGSWallet* getZWallet() { return zwalletMain; }
    ...
    void setZSgsAutoBackups(bool fEnabled)
    {
    ...
    /** zSGS reset */
    boost::signals2::signal<void()> NotifyzSGSReset;
