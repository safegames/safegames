...
#if defined(NDEBUG)
#error "SAFEGAMES cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usgs) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zSGS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSGS spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSGS without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSGS spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zSGS spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSGS spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsgs");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zSGS mints are not already known
...
else if (nHeight <= 200) { nSubsidy = 2 * COIN; }
else if (nHeight <= 20000) { nSubsidy = 8.75 * COIN; }
else if (nHeight <= 30000) { nSubsidy = 10 * COIN; }
else if (nHeight <= 40000) { nSubsidy = 11.25 * COIN; }
else if (nHeight <= 50000) { nSubsidy = 12.5 * COIN; }
else if (nHeight <= 60000) { nSubsidy = 13.75 * COIN; }
else if (nHeight <= 70000) { nSubsidy = 15 * COIN; }
else if (nHeight <= 80000) { nSubsidy = 16.25 * COIN; }
else if (nHeight <= 90000) { nSubsidy = 17.5 * COIN; }
else if (nHeight <= 100000) { nSubsidy = 18.75 * COIN; }
else if (nHeight <= 110000) { nSubsidy = 20 * COIN; }
else if (nHeight <= 120000) { nSubsidy = 21.25 * COIN; }
else if (nHeight <= 130000) { nSubsidy = 22.5 * COIN; }
else if (nHeight <= 140000) { nSubsidy = 23.75 * COIN; }
else if (nHeight <= 150000) { nSubsidy = 25 * COIN; }
else if (nHeight <= 160000) { nSubsidy = 26.25 * COIN; }
else if (nHeight <= 170000) { nSubsidy = 27.5 * COIN; }
else if (nHeight <= 180000) { nSubsidy = 28.75 * COIN; }
else if (nHeight <= 190000) { nSubsidy = 30 * COIN; }
else if (nHeight <= 200000) { nSubsidy = 31.25 * COIN; }
else if (nHeight <= 210000) { nSubsidy = 32.5 * COIN; }
else if (nHeight <= 220000) { nSubsidy = 33.75 * COIN; }
else if (nHeight <= 230000) { nSubsidy = 35 * COIN; }
else if (nHeight <= 240000) { nSubsidy = 36.25 * COIN; }
else if (nHeight <= 250000) { nSubsidy = 37.5 * COIN; }
else if (nHeight <= 260000) { nSubsidy = 38.75 * COIN; }
else if (nHeight <= 270000) { nSubsidy = 40 * COIN; }
else if (nHeight <= 280000) { nSubsidy = 39.5 * COIN; }
else if (nHeight <= 290000) { nSubsidy = 39 * COIN; }
else if (nHeight <= 300000) { nSubsidy = 38.5 * COIN; }
else if (nHeight <= 310000) { nSubsidy = 38 * COIN; }
else if (nHeight <= 320000) { nSubsidy = 37.5 * COIN; }
else if (nHeight <= 330000) { nSubsidy = 37 * COIN; }
else if (nHeight <= 340000) { nSubsidy = 36.5 * COIN; }
else if (nHeight <= 350000) { nSubsidy = 36 * COIN; }
else if (nHeight <= 360000) { nSubsidy = 35.5 * COIN; }
else if (nHeight <= 370000) { nSubsidy = 35 * COIN; }
else if (nHeight <= 380000) { nSubsidy = 34.5 * COIN; }
else if (nHeight <= 390000) { nSubsidy = 34 * COIN; }
else if (nHeight <= 21000000) { nSubsidy = 33.5 * COIN; }
else { nSubsidy = 0 * COIN; }

    return nSubsidy;
}

int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSGSStake)
{
    int64_t ret = 0;
    ...
    if (nHeight < 2) { ret = 0; } 
	else if (nHeight <= 200) { ret = blockValue * 1; }
    ...
    else if (nHeight <= 60000) { ret = blockValue * 0.77; }
    else if (nHeight <= 70000) { ret = blockValue * 0.775; }
    else if (nHeight <= 80000) { ret = blockValue * 0.78; }
    else if (nHeight <= 90000) { ret = blockValue * 0.785; }
    else if (nHeight <= 100000) { ret = blockValue * 0.79; }
    else if (nHeight <= 110000) { ret = blockValue * 0.795; }
    else if (nHeight <= 120000) { ret = blockValue * 0.8; }
    else if (nHeight <= 130000) { ret = blockValue * 0.805; }
    else if (nHeight <= 140000) { ret = blockValue * 0.81; }
    else if (nHeight <= 150000) { ret = blockValue * 0.815; }
    else if (nHeight <= 160000) { ret = blockValue * 0.82; }
    else if (nHeight <= 170000) { ret = blockValue * 0.825; }
    else if (nHeight <= 180000) { ret = blockValue * 0.83; }
    else if (nHeight <= 190000) { ret = blockValue * 0.835; }
    else if (nHeight <= 200000) { ret = blockValue * 0.84; }
    else if (nHeight <= 210000) { ret = blockValue * 0.845; }
    else if (nHeight <= 220000) { ret = blockValue * 0.85; }
    else if (nHeight <= 230000) { ret = blockValue * 0.855; }
    else if (nHeight <= 240000) { ret = blockValue * 0.86; }
    else if (nHeight <= 250000) { ret = blockValue * 0.865; }
    else if (nHeight <= 260000) { ret = blockValue * 0.87; }
    else if (nHeight <= 270000) { ret = blockValue * 0.875; }
    else if (nHeight <= 280000) { ret = blockValue * 0.88; }
    else if (nHeight <= 290000) { ret = blockValue * 0.885; }
    else if (nHeight <= 300000) { ret = blockValue * 0.89; }
    else if (nHeight <= 310000) { ret = blockValue * 0.895; }
    else if (nHeight <= 320000) { ret = blockValue * 0.89; }
    else if (nHeight <= 330000) { ret = blockValue * 0.885; }
    else if (nHeight <= 340000) { ret = blockValue * 0.88; }
    else if (nHeight <= 350000) { ret = blockValue * 0.875; }
    else if (nHeight <= 360000) { ret = blockValue * 0.87; }
    else if (nHeight <= 370000) { ret = blockValue * 0.865; }
    else if (nHeight <= 380000) { ret = blockValue * 0.86; }
    else if (nHeight <= 390000) { ret = blockValue * 0.855; }
    else if (nHeight <= 21000000) { ret = blockValue * 0.85; }
    else { ret = 0; }
    return ret;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SAFEGAMES
...
{
    RenameThread("safegames-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSGSMinted()
{
...
void RecalculateZSGSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSGS supply
...
bool RecalculateSGSSupply(int nHeightStart)
{
...
{
    // SAFEGAMES: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZSGSSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZSGSSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSGS supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSgsSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SGS to zSGS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSGS spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSGS serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSGS stake block is less than allowed start height", __func__);

    if (CZSgsStake* zSGS = dynamic_cast<CZSgsStake*>(stake)) {
        CBlockIndex* pindexFrom = zSGS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSGS stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSGS stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSGS->GetValue()));
        if (nChecksum200 != zSGS->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSGS->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSGS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSGS fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSGS mints and %d zSGS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SAFEGAMES: We use certain sporks during IBD, so check to see if they are
