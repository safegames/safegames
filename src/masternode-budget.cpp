...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 SGS finalization collateral
            //       will also cover the old 50 SGS finalization collateral.
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    LogPrint("MASTERNODE","REALLYYYYYYYYYYYYYY=========================>%f\n", nSubsidy / COIN);
    // Amount of blocks in a months period of time (using 1 minutes per) = (60*24*30)
    if (nHeight <= 172800) {
        return 648000 * COIN;
    } else {
