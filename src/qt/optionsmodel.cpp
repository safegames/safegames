...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SGS);
    ...
    if (!settings.contains("nAnonymizeSafegamesAmount"))
        settings.setValue("nAnonymizeSafegamesAmount", 1000);

    nAnonymizeSafegamesAmount = settings.value("nAnonymizeSafegamesAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:55000");
    ...
    if (settings.contains("nAnonymizeSafegamesAmount"))
        SoftSetArg("-anonymizesafegamesamount", settings.value("nAnonymizeSafegamesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in safegames.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSafegamesAmount:
            return QVariant(nAnonymizeSafegamesAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSafegamesAmount:
            nAnonymizeSafegamesAmount = value.toInt();
            settings.setValue("nAnonymizeSafegamesAmount", nAnonymizeSafegamesAmount);
            emit anonymizeSafegamesAmountChanged(nAnonymizeSafegamesAmount);
            break;
        case CoinControlFeatures:
