...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered safegames address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SGS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal safegames:URI (safegames:XyZ...?message=example)
            {
...
static void NotifyzSGSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSGSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSGSReset.connect(boost::bind(NotifyzSGSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSGSReset.disconnect(boost::bind(NotifyzSGSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsgsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
