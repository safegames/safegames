...
#include "ui_zsgscontroldialog.h"
...
std::set<std::string> ZSgsControlDialog::setSelectedMints;
std::set<CMintMeta> ZSgsControlDialog::setMints;

ZSgsControlDialog::ZSgsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSgsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSgsControlDialog::~ZSgsControlDialog()
{
    delete ui;
}

void ZSgsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSgsControlDialog::updateList()
{
...
void ZSgsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSgsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSgs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSgsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSgsControlDialog::GetSelectedMints()
{
...
void ZSgsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
