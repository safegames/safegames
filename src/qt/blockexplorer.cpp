...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::SGS, nValue);
...
{
    std::string genesisblockhash = "0000012cff8e9a83e57588462668f5cb48a579b7466c89d30e305d3935b56cfa";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (safegames.conf).");
            QMessageBox::warning(this, "SAFEGAMES Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
