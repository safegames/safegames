...
#ifndef SAFEGAMES_ZSGSWALLET_H
#define SAFEGAMES_ZSGSWALLET_H
...
public:
    CzSGSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSGS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSGS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SAFEGAMES_ZSGSWALLET_H
