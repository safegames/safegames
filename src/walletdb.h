...
public:
    ...
    bool WriteZSGSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSGSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSGSSeed_deprecated(uint256& seed);
    bool EraseZSGSSeed();
    bool EraseZSGSSeed_deprecated();

    bool WriteZSGSCount(const uint32_t& nCount);
    bool ReadZSGSCount(uint32_t& nCount);
