...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
		...
		//pblock->vtx[0].vin[0].scriptSig = CScript() << nHeight << OP_0; //EK ==> STEP 1
        if (!fProofOfStake) {
            pblock->vtx[0] = txNew;
			pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight); //EK ==> STEP 2
  ...
  //      uint256 nCheckpoint;
		//uint256 hashBlockLastAccumulated = chainActive.Height() + 1);// chainActive[nHeight - (nHeight % 10) - 10]->GetBlockHash();
