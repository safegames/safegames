...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    bool fZerocoinActive2 = nHeight >= Params().Zerocoin_Block_V2_Start();
    if (fZerocoinActive2)
        pblock->nVersion = 4;
    else
        pblock->nVersion = 3;
    ...
    if (fProofOfStake) {
        ...
        //CBlockIndex* pindexPrev = chainActive.Tip(); //SGS removed
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock);
...

    {
        ...
        //CBlockIndex* pindexPrev = chainActive.Tip(); // ek new
        //const int nHeight = pindexPrev->nHeight + 1; // ek new
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zsgss that have been in mempool long
                    //and higher priority to zsgss that are large in value
                    ...
                    // zSGS spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zSGS spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                for (const CTxIn& txIn : tx.vin) { //ek new added &
                ...
                //This zSGS serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
            ...
            for (const CBigNum& bnSerial : vTxSerials) //ek added &
        ...
        if (!fProofOfStake) {
            pblock->vtx[0] = txNew;
			//if (pindexPrev->nHeight < 992)// Added
			//	pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight); //Specific to safegames.
		...
		if (fZerocoinActive) {
			...
			if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
				//For the period before v2 activation, zSGS will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zSGS with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zSGS key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zSGS key failed \n");
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SAFEGAMESMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zsgsTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("SAFEGAMESMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SAFEGAMESMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("safegames-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SAFEGAMESMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
